...
def summarize(path: str, findings: list[Finding]) -> dict:
    ...
    return {
        ...
        "scope_note": (
            "Static analysis of Python source, dependency manifests, config/infra "
            "files and encrypted-artifact headers (v1). Reports quantum-vulnerable "
            "cryptography by CBOM classification (Shor/Grover) with a suggested "
            "migration target per finding; it does not decrypt data, execute code, "
            "or inspect runtime, binaries, or live endpoints."
        ),
    }
